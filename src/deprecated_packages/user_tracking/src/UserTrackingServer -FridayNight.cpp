...
//		ROS_INFO("%s Got Navigation Server...", action_name.c_str());
		...
		head_pos_sub_ = nh_.subscribe<drrobot_h20_player::HeadCmd>("head_pose", 1, boost::bind(&UserTrackingAction::neckPositionCB, this, _1));
		marker_coordinates_sub_ = nh_.subscribe<visualization_msgs::Marker>("tracked_users_markers" , 1000, boost::bind(&UserTrackingAction::markerCoordinatesCB, this, _1));
	...
	bool isIncreasing(int array_size)
	{
		float past_xtion_y[5];
		bool var = true;
		
		for (int i = 0; i < array_size - 1; i++)
		{
			//Copy values of global variable
			past_xtion_y[i] = past_coordinates_y[i];
			
			//Get the "absolute value" of the array
			if (past_xtion_y[i] < 0)
			{
				past_xtion_y[i] = past_xtion_y[i] * -1;
			}
			
			//Check if values are always increasing
			//is oldest > newest?
			if (past_xtion_y[i] > past_xtion_y[i+1])
			{
				 var = false;
			}
		}
...
//			while (marker_user_id == -1);
			...
			if (!isPreempted)
			{
				...
				{
                    if (feedback->users_identified[i] == user_name && !user_name.empty())
					{
				...
				if (foundUser)
				{
					...
					if (shouldmove)
					{
						...
						{
/*		        				//Predictive movement turning
							
							//if there is no new data from sensor within 3 seconds, set turn to false
							//if (predictive_count >= 3)
							if (last_message_received >= ros::Duration(3))
							{
								//predictive_count = 0;
								predictive_turn = false;
							}
							...
							if (predictive_turn)
							{
								//check if absolute value of past_coordinates_y is continually increasing
								if (isIncreasing(counter))
								{
									ROS_INFO("counter: %d", counter);
									ROS_INFO("past_coordinates_y[%d]: %f", counter, past_coordinates_y[counter]);
									//Check last value to determine direction of turn
									if(past_coordinates_y[counter] > 0)
									{
										ROS_INFO("Predictive Turn Left");
										maxTurn = 0.4;
									}
									else
									{
										ROS_INFO("Predictive Turn Right");
										maxTurn = -0.4;
									}
								}
							}
*/		        			}
						...
						if (user_id_xtion > -1)
						{
							...
							//past_coordinates_x[counter] = users_info_old[user_id_xtion].x;
							past_coordinates_y[counter] = users_info_old[user_id_xtion].y;
							//ROS_INFO("past_coordinates_x[%d]: %f", counter, past_coordinates_x[counter]);
							ROS_INFO("past_coordinates_y[%d]: %f", counter, past_coordinates_y[counter]);
							counter++;
							if (counter >= 5)
							{
...
private:
	...
	bool predictive_turn;
	int counter;
	//float past_coordinates_x[5];
	float past_coordinates_y[5];
...
int main (int argc, char **argv)
{
	...
	ROS_INFO("User Tracking Server is running");
