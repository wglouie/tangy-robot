  ...
  if(already_tracking==false)
  {
	  ...
	  //loop until a personofinterest is found. this occurs when there is no one on the screen.
	      ...
	      //what is this for???????? coment it if it does not work
              ...
              //ROS_INFO("x: %d y: %d",pos_of_face[0],pos_of_face[1]);
	      ...
	      //ROS_INFO("THE POS x:%d y:%d",pos_of_face[0],pos_of_face[1]);
	      ...
	      //ROS_INFO(" THE POS WITH CENTER BASE x: %d, y: %d",pos_of_face_center_base[0],pos_of_face_center_base[1]);
...
/*slow way
	      if(pos_of_face_center_base[0]>20)
		  neckHandler.increment_dir(-20,0);
	      if(pos_of_face_center_base[0]<-20)
		  neckHandler.increment_dir(20,0);
	      if(pos_of_face_center_base[1]>20)
		  neckHandler.increment_dir(0,20);
	      if(pos_of_face_center_base[1]<-20)
		  neckHandler.increment_dir(0,-20);
*/
