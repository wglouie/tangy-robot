...
// Copyright   :
// Description : Receives a position and find an ik solution
//============================================================================
...
public:
	...
	{	
		...
		//Publish commands and start server
		cmd_arm_pub = nh_.advertise<drrobot_h20_arm_player::ArmCmd>("cmd_arm", 1);
		as_.start();
			
	}
	
	//Given a positon find IK solution
	...
	{ 	
		...
		const geometry_msgs::Pose &position = pose;
	  	...
	  	if (found_ik)
	  	{
	  		...
	  		//const Eigen::Affine3d &end_effector_state = joint_state_group->getRobotState()->getLinkState("wrist_right_arm_x_revolute")->getGlobalLinkTransform();
			...
			{
				if(i == 0) ang[i] = -1.5707;
				else if(i == 1) ang[i] = -1.5707;
		  		else if(i > 1) ang[i] = right_joint_values[i - 2];
			}
			
			for(int j=8; j < 16; j++)
			{
				if(j == 8) ang[j] = -1.5707;
				else if(j == 9) ang[j] = -1.5707;
		  		else if(j > 9) ang[j] = left_joint_values[j - 10];
			}
